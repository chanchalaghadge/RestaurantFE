...
export const menuItemsApi = {
  list: (filters: { search?: string; categoryId?: number; status?: string } = {}) => api<MenuItemApi[]>(`/api/menu-items${query(filters)}`), get: (id: number) => api<MenuItemApi>(`/api/menu-items/${id}`),
