  ...
  return <section className="menu-item-page">
    ...
    <form className="menu-item-form" onSubmit={submit}>
      <div className="menu-item-top-grid">
        ...
        <section className="menu-panel additional-panel"><h2>Item Image</h2>
          ...
          <h2>Additional Details</h2><label>Preparation Time (minutes)<input type="number" min="0" value={form.preparationTimeMinutes} onChange={(event) => update("preparationTimeMinutes", Number(event.target.value))} /></label><label>Calories<input type="number" min="0" value={form.calories ?? ""} onChange={(event) => update("calories", event.target.value ? Number(event.target.value) : undefined)} /></label><label>Ingredients<textarea value={form.ingredients ?? ""} onChange={(event) => update("ingredients", event.target.value)} /></label><label>Tags<input value={form.tags ?? ""} onChange={(event) => update("tags", event.target.value)} placeholder="Comma-separated tags" /></label>
        </section>
      </div>
