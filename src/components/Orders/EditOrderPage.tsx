...
function EditOrderPage() {
  ...
  return <section className="edit-order-page"><form onSubmit={save}>
    <header className="edit-order-header"><div className="edit-order-header-main"><Breadcrumb items={[{ label: "Orders", path: "/orders" }, { label: `Order #${order.id}`, path: `/orders/${order.id}` }, { label: "Edit" }]} /><h1><button type="button" onClick={() => navigate(`/orders/${order.id}`)} aria-label="Back to order">←</button> Edit Order #{order.id}</h1></div><div className="edit-order-header-actions"><button type="button" className="secondary-button" onClick={() => navigate(`/orders/${order.id}`)}>Cancel</button><button className="primary-button" disabled={saving}>{saving ? "Updating..." : "Update Order"}</button></div></header>
    <p className="kot-addition-note">New items added here are sent to the kitchen as a separate KOT ticket; existing ready items stay on their original ticket.</p>
